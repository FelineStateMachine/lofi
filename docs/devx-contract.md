...
| DX-AUTH-01      | Identity wording matches custody/recovery.                     | UI distinguishes the local account, recoverable passkey, local phrase guard, and phrase bearer secret; restore confirms replacement and states provider/RP-ID limits. | #8 history and generated golden | yes     | validated |
| DX-DEVICE-01    | Device/auth tests use a stable, appropriately isolated origin. | RP-ID follows the exact stable hostname; credential retrieval succeeds there and a sibling RP is rejected without storing secret material.                            | #4, #8, M3, golden gate         | yes     | validated |
| DX-DEVICE-UX-01 | Device preview is one productized command.                     | Primary path prints stable URL, capability report, and remediation.                                                                                                   | M3                              | no      | validated |
| DX-OFFLINE-01   | Installed production cold-start renders retained data offline. | Installed PWA retains a local write across reload and airplane-mode cold start.                                                                                       | feasibility #4 and M3           | no      | validated |
...
an author-facing concept. Ignored `.lofi/` tooling files and `dist/sw.js` are regenerated from the
pinned package; they are not author-editing surfaces.
...
| `schema.ts`              | Application schema declarations.                              | Declared through `@nzip/lofi/schema`, the curated one-to-one re-export of the pinned Jazz 2 DSL; see `docs/decisions/schema-facade-alpha53.md`. |
| `permissions.ts`         | Application access policy next to the schema.                 | Declared through the same `@nzip/lofi/schema` surface; Jazz tooling watches this location.                                                      |
...
| Package runtime          | Vendor clients, workers, storage, sync, auth, and PWA wiring. | May change across package pins without changing product UI.                                                                                     |
| Escape-hatch module      | Explicitly isolated unsupported vendor access.                | Never generated by default; package upgrades provide no compatibility promise for vendor internals.                                             |

## Safe defaults

- **Schema:** expose the pinned Jazz 2 DSL through the `@nzip/lofi/schema` re-export — identical
  names and semantics, curated to the tested surface; never invent a second schema language.
...
M2 graduated the single-package command surface, generator, development inspector, and
readiness-first browser-testing helpers. M3 graduated representative mobile PWA validation, and the
current generated-app release gate covers install/update output, package-owned runtime seams,
recoverable account replacement, and collaboration policy checks. A second independent consumer
