...
Deno.test("createProject materializes the complete starter snapshot", async () => {
  ...
  try {
    ...
    assertEquals(config.imports["@nzip/lofi"], "jsr:@nzip/lofi@0.4.0");
    assertEquals(config.imports["@nzip/lofi/"], "jsr:@nzip/lofi@0.4.0/");
    assertEquals(config.imports["@nzip/lofi/testing"], "jsr:@nzip/lofi@0.4.0/testing");
    const lofiSpecifiers = Object.entries(config.imports)
      .filter(([name]) => name.startsWith("@nzip/lofi/"))
      .map(([, specifier]) => String(specifier));
    assert(
      lofiSpecifiers.length === 18,
      `expected one package prefix, access, two integrations, six commands, seven recipes, and testing, received ${lofiSpecifiers.length}`,
    );
    assert(
      lofiSpecifiers.every((specifier) => specifier.startsWith("jsr:@nzip/lofi@0.4.0/")),
...
Deno.test("package manifest and generated version stay coupled", () => {
  ...
  assertEquals(
    packageManifest.exports["./recipes/scope-extension"],
  ...
  assertEquals(
    packageManifest.exports["./recipes/window-controls-overlay"],
