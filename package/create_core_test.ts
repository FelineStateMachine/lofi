...
Deno.test("createProject materializes the complete starter snapshot", async () => {
  ...
  try {
    ...
    assertEquals(config.imports["@nzip/lofi"], "jsr:@nzip/lofi@0.7.0");
    assertEquals(config.imports["@nzip/lofi/"], "jsr:@nzip/lofi@0.7.0/");
    assertEquals(config.imports["@nzip/lofi/testing"], "jsr:@nzip/lofi@0.7.0/testing");
    const lofiSpecifiers = Object.entries(config.imports)
      .filter(([name]) => name.startsWith("@nzip/lofi/"))
      .map(([, specifier]) => String(specifier));
    assert(
      lofiSpecifiers.length === 19,
    ...
    assert(
      lofiSpecifiers.every((specifier) => specifier.startsWith("jsr:@nzip/lofi@0.7.0/")),
